        ...
        child: Column(
          children: [
            if (provider.state is Success)
              Builder(builder: (context) {
                WidgetsBinding.instance.addPostFrameCallback((_) {
                  final success = provider.state as Success;
                  if (provider.autoSignIn) {
                    Navigation.pushNamedAndRemoveUntil(Routes.home, arguments: success);
                  } else {
            ...
            Row(
              children: [
                Checkbox(
                  value: provider.autoSignIn,
                  onChanged: (value) => provider.toggleAutoSignIn(value ?? false),
                ),
                PrimaryText("Auto sign in after register"),
              ],
            ),
            12.verticalSpace,
            PrimaryTextFormField(
